...
def generate_html_report(
    ...
    """
    try:
        fix_plan = _ensure_fix_plan(metrics, codebase_path, fix_plan)
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")
        sections = _generate_report_sections(metrics, fix_plan)
        return _assemble_html_document(codebase_path, timestamp, sections)
    except Exception as e:
        return f"<!-- Error generating HTML report: {str(e)} -->"


def _ensure_fix_plan(
    metrics: CodebaseMetrics,
    codebase_path: str,
    fix_plan: Optional[FixPlan],
) -> FixPlan:
    """Ensure a fix plan exists, generating it if necessary."""
    if fix_plan is None:
        return generate_fix_plan(codebase_path, metrics)
    return fix_plan


def _generate_report_sections(metrics: CodebaseMetrics, fix_plan: FixPlan) -> dict:
    """Generate all content sections for the report."""
    return {
        "health": _generate_health_section(metrics),
        "metrics": _generate_metrics_section(metrics),
        "fixes": _generate_fixes_section(fix_plan),
        "smells": _generate_smells_section(metrics),
        "languages": _generate_languages_section(metrics),
        "duplicates": _generate_duplicates_section(metrics),
    }
...
def _assemble_html_document(
    ...
    """Assemble the final HTML document within the report structure."""
    return f'''<!DOCTYPE html>
<html lang="en">
{_generate_html_head(codebase_path)}
<body>
    {_generate_html_header(codebase_path, timestamp)}
...
def _generate_html_head(codebase_path: str) -> str:
    """Generate HTML head section."""
    return f'''<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Revibe Report — {escape(codebase_path)}</title>
    <style>
{_get_styles()}
    </style>
</head>'''
...
def _generate_html_header(codebase_path: str, timestamp: str) -> str:
...
def _generate_metrics_section(metrics: CodebaseMetrics) -> str:
    ...
    test_target = "✅ Good" if test_ratio_pct >= 80 else "⚠️ Low" if test_ratio_pct >= 50 else "🔴 Critical"
...
def _generate_fixes_section(fix_plan: FixPlan) -> str:
    ...
    if not fix_plan.fixes:
        return '''
        <section class="section">
            <h2 class="section-title">✅ No Critical Fixes Needed</h2>
            <div class="card">
...
''')
    ...
    return f'''
        <section class="section">
            <h2 class="section-title">🔧 Fix Instructions</h2>
...
''')
    ...
    return f'''
        <section class="section">
            <h2 class="section-title">🤖 AI Code Smells</h2>
            <div class="card">
                {"".join(smell_bars)}
            </div>
        </section>
...
''')
    ...
    return f'''
        <section class="section">
            <h2 class="section-title">📋 Duplicate Files ({len(metrics.duplicate_groups)} groups)</h2>
